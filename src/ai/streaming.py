...
1. 当你需要生成新的可视化代码时，务必将图片保存为'answer.svg'
2. 如果你需要修改现有代码，请保留代码的整体结构，图片保存为'answer.svg'
3. 确保使用相对路径保存图片，不要使用绝对路径
4. 确保代码能独立运行，必须包含导入必要的库、读取数据和保存图片的完整过程，使用matplotlib，不要使用seaborn等库

请记住：所有可视化代码必须将结果保存为'answer.svg'，这一点非常重要。最终的代码会由系统自动执行，图片路径会被系统自动修改和管理。
"""
